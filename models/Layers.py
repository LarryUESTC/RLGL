...
class GNN_pre_Model(nn.Module):
    def __init__(self, n_in, cfg=None, batch_norm=False, act='relu', gnn='GCN', dropout=0.0, final_mlp=0):
        super(GNN_pre_Model, self).__init__()
        ...
        self.sparse = True
        self.cfg = cfg
        self.layer_num = len(cfg) - 1
        GCN_layers = []
        act_layers = []
        bat_layers = []
        in_channels = n_in

        self.get_embedding_layer = nn.Sequential(
            nn.Linear(in_channels, cfg[0], bias=True),
            act_layer(act)
        )
        in_channels = cfg[0]
    ...
    def get_embedding(self, A_a, X_a):
        X_a = self.get_embedding_layer(X_a)
        for i in range(self.layer_num):
            X_a = self.GCN_layers[i](A_a, X_a)
            if self.act:
                X_a = self.act_layers[i](X_a)
    ...
    def get_first_embedding(self, X_a):
        return self.get_embedding_layer(X_a)
